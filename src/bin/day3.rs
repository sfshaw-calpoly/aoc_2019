...
enum Direction{Vert, Horz}
#[derive(Debug)]
struct Line {start:Point, end:Point, dir: Direction}
impl Line {
    fn left(&self) -> isize{
        isize::min(self.start.x, self.end.x)
    }
    fn right(&self)-> isize{
        isize::max(self.start.x, self.end.x)
    }fn bottom(&self)-> isize{
        isize::min(self.start.y, self.end.y)
    }
    fn top(&self)-> isize{
        isize::max(self.start.y, self.end.y)
    }
}
...
fn main() {
    ...
    // let inputs: Vec<&str> = vec!["R8,U5,L5,D3", "U7,R6,D4,L4"];
    let mut lines_sets: Vec<Vec<Line>> = inputs.iter()
                                                .map(|s| get_lines(s.split(delim)
                                                                    .collect()))
                                                .collect();
  

    let set_a = lines_sets.pop().unwrap();
    let set_b = lines_sets.pop().unwrap();

    let mut intersections: Vec<Point> = vec!();

    for l_a in set_a.iter() {
        for l_b in set_b.iter() {
            
            match intersects_huh(l_a, l_b) {
...
fn intersects_huh(a: &Line, b: &Line) -> Option<Point> {
    if a.dir ==Direction::Horz &&
        b.dir == Direction::Vert &&
        a.left() <= b.left() &&
        a.right() >= b.right() &&
        a.top() <= b.top() &&
        a.bottom() >= b.bottom()
    {
        let p = Point{x: b.left(), y: a.top()};
        Some(p)
    } else if a.dir == Direction::Vert &&
        b.dir == Direction::Horz &&
        a.left() >= b.left() &&
        a.right() <= b.right() &&
        a.top() >= b.top() &&
        a.bottom() <= b.bottom()  
    {
        Some(Point{x: a.left(), y: b.top()})
    } else { None }
}
...
fn get_lines(wire: Vec<&str>)-> Vec<Line>{
    ...
    for s in wire {
        let dir = &s[0..1];
        let magnitude = s[1..].parse::<isize>().unwrap();
        let (dir, x, y) : (Direction, isize, isize) = match dir.as_ref(){
            "R" => Ok((Direction::Horz, 1, 0)),
            "U" => Ok((Direction::Vert, 0, 1)),
            "L" => Ok((Direction::Horz, -1, 0)),
            "D" => Ok((Direction::Horz, 0, -1)),
            &_ => Err("RIP")
        }.unwrap();

        let new_pos = Point{x: pos.x + (x * magnitude) as isize, y: pos.y + (y * magnitude) as isize};
        lines.push(Line {start: pos.clone(), end: new_pos.clone(), dir: dir});
        pos = new_pos;
    }
    return lines;
}
